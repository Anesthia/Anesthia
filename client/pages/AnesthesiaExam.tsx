...
export default function AnesthesiaExam() {
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <h3 className="text-lg font-semibold text-gray-900 flex items-center space-x-2">
                      ...
                      <span>👁️ Skóra i Błony Śluzowe</span>
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {[
                        "Skóra różowa, ciepła, wilgotna",
                        ...
                        "Brak obrzęków",
                        "Źrenice okrągłe, równe, reagujące na światło"
                      ].map((finding) => (
                        <div key={finding} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50">
                          <Checkbox
                            id={`skin-${finding}`}
                            checked={physicalExamFindings.skin.includes(finding)}
                            onCheckedChange={(checked) =>
                              handlePhysicalExamFindingChange('skin', finding, checked as boolean)
                            }
                            className="scale-110"
                          />
                          <Label htmlFor={`skin-${finding}`} className="text-sm font-normal flex-1 cursor-pointer">
                            {finding}
                          </Label>
                        </div>
                      ))}
                  ...
                  <div className="space-y-4">
                    <h3 className="text-lg font-semibold text-gray-900 flex items-center space-x-2">
                      ...
                      <span>🔍 Inne Obserwacje</span>
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {[
                        "Stan odżywienia prawidłowy",
                        "Wygląd ogólny dobry",
                        "Chód samodzielny bez wsparcia",
                        "Brak deformacji klatki piersiowej",
                        "Brzuch miękki, niebolesny",
                        "Ograniczenia ruchomości kręgosłupa",
                        "Protezy zębowe w jamie ustnej",
                        "Ślady po operacjach"
                      ].map((finding) => (
                        <div key={finding} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50">
                          <Checkbox
                            id={`other-${finding}`}
                            checked={physicalExamFindings.other.includes(finding)}
                            onCheckedChange={(checked) =>
                              handlePhysicalExamFindingChange('other', finding, checked as boolean)
                            }
                            className="scale-110"
                          />
                          <Label htmlFor={`other-${finding}`} className="text-sm font-normal flex-1 cursor-pointer">
                            {finding}
                          </Label>
                        </div>
                      ))}
                  ...
                  {(physicalExamFindings.cardiovascular.length > 0 ||
                    physicalExamFindings.respiratory.length > 0 ||
                    physicalExamFindings.nervous.length > 0 ||
                    physicalExamFindings.skin.length > 0 ||
                    physicalExamFindings.other.length > 0) && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      <h4 className="font-semibold text-blue-900 mb-3">📋 Podsumowanie Wybranych Obserwacji:</h4>
                      <div className="space-y-2 text-sm text-blue-800">
                        {physicalExamFindings.cardiovascular.length > 0 && (
                          <p><strong>Układ krążenia:</strong> {physicalExamFindings.cardiovascular.join(', ')}</p>
                        )}
                        {physicalExamFindings.respiratory.length > 0 && (
                          <p><strong>Układ oddechowy:</strong> {physicalExamFindings.respiratory.join(', ')}</p>
                        )}
                        {physicalExamFindings.nervous.length > 0 && (
                          <p><strong>Układ nerwowy:</strong> {physicalExamFindings.nervous.join(', ')}</p>
                        )}
                        {physicalExamFindings.skin.length > 0 && (
                          <p><strong>Skóra i błony śluzowe:</strong> {physicalExamFindings.skin.join(', ')}</p>
                        )}
                        {physicalExamFindings.other.length > 0 && (
                          <p><strong>Inne obserwacje:</strong> {physicalExamFindings.other.join(', ')}</p>
                        )}
                      </div>
                    </div>
                  )}
